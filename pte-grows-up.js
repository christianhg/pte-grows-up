...
export default {
  render(container, ctx) {
      ...
      return `
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        @media (max-width:700px) {
          ...
          .rc h2 { font-size:12px; margin:24px 0 10px; }
    ...
    const blockKeys = PT.map(b => b._key);
    ...
    _handleKey = function handleKey(e) {
      if (e.key === 'Escape') {
        if (selectedKey) { select(selectedKey, 'keyboard'); } // toggle off
        return;
      }
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        ...
        let next;
        if (e.key === 'ArrowDown') {
          next = idx < blockKeys.length - 1 ? idx + 1 : 0;
        } else {
          next = idx > 0 ? idx - 1 : blockKeys.length - 1;
        }
        select(blockKeys[next], 'keyboard');
      }
    };
    // Register once — outside wire() so theme toggles don't duplicate
  ...
  cleanup() {
    if (_handleKey) document.removeEventListener('keydown', _handleKey);
  }
}
